...
function handleDocumentClick(event) {
  ...
  const defineAmount = button.closest('[data-define-amount]');
  if (defineAmount) {
    openQuickAmountDialog(defineAmount.dataset.defineAmount, defineAmount.dataset.id);
    return;
  }
...
function handleDocumentChange(event) {
  ...
  if (event.target.id === 'recordRecurring' || event.target.name === 'recordFrequency') {
    updateRecordRecurrenceUI();
  }

  if (event.target.name === 'flowEntryDirection') {
    updateFlowEntryUI();
  }
...
function renderDashboard() {
  ...
  const pendingWithHistory = totals.expenseOpen + previousPending.reduce((total, item) => total + expenseOpenAmount(item), 0);
...
function renderExpenseList() {
  ...
  items.sort((a, b) => expenseSortDay(a) - expenseSortDay(b) || a.title.localeCompare(b.title));
...
function settledFlowAmount(type, item, database = db) {
  return roundMoney(flowEntries(type, item.id, database)
    .filter(entry => flowEntryDirection(entry) === 'decrease')
    .reduce((total, entry) => total + Number(entry.amount || 0), 0));
}

function increasedFlowAmount(type, item, database = db) {
  return roundMoney(flowEntries(type, item.id, database)
    .filter(entry => flowEntryDirection(entry) === 'increase')
    .reduce((total, entry) => total + Number(entry.amount || 0), 0));
}

function accumulatedFlowAmount(type, item, database = db) {
  return roundMoney(Number(item.amount || 0) + increasedFlowAmount(type, item, database));
}

function remainingFlowAmount(type, item, database = db) {
  return Math.max(0, roundMoney(accumulatedFlowAmount(type, item, database) - settledFlowAmount(type, item, database)));
}

function flowEntryDirection(entry) {
  return entry?.direction === 'increase' ? 'increase' : 'decrease';
}
...
function isWeeklyExpense(item) {
  return item?.recurrenceFrequency === 'weekly';
}

function weekdayLabel(value) {
  return WEEKDAY_LABELS[Number(value)] || WEEKDAY_LABELS[1];
}

function weeklyOccurrenceDays(monthValue, weekday) {
  const [year, month] = String(monthValue || currentMonth()).split('-').map(Number);
...
function weeklyExpectedCountForItem(item) {
  return weeklyOccurrenceDays(item?.month, item?.weeklyDay).length;
}

function weeklyPayments(item, includeDeleted = false) {
  const entries = Array.isArray(item?.weeklyPayments) ? item.weeklyPayments : [];
  return entries.filter(entry => includeDeleted || !entry.deletedAt);
}
...
function weeklyDueCountThrough(item, referenceDate = new Date()) {
  const [year, month] = String(item?.month || currentMonth()).split('-').map(Number);
  if (!year || !month) return 0;
  return weeklyOccurrenceDays(item.month, item.weeklyDay).filter(day => (
    new Date(year, month - 1, day, 23, 59, 59, 999) < referenceDate
  )).length;
}
...
function expensePaidAmount(item) {
  if (isWeeklyExpense(item)) {
    return Math.min(Number(item.amount || 0), roundMoney(weeklyPaidCount(item) * Number(item.weeklyAmount || 0)));
  }
  return item.status === 'paid' ? Number(item.amount || 0) : 0;
}

function expenseOpenAmount(item) {
  return Math.max(0, roundMoney(Number(item.amount || 0) - expensePaidAmount(item)));
}
...
function reconcileWeeklyExpenseStatus(item, touch = false) {
  ...
  if (touch && statusChanged) {
    ...
    item.statusRevision = Number(item.statusRevision || 0) + 1;
  }
}
...
function calculateTotals(month = state.month) {
  ...
  const expensePaid = roundMoney(expenses
    .filter(item => !isCardExpense(item))
    .reduce((total, item) => total + expensePaidAmount(item), 0));
  const expenseOpen = roundMoney(expenses
    .filter(item => !isCardExpense(item))
    .reduce((total, item) => total + expenseOpenAmount(item), 0));
...
function openRecordDialog(type, id = '') {
  ...
  $('#recordWeekday').value = String(item?.weeklyDay ?? recurrence?.weeklyDay ?? 1);
  ...
  $$('input[name="recordFrequency"]').forEach(input => { input.checked = input.value === frequency; });
...
function saveRecordFromForm(event) {
  ...
  if (type === 'account') {
    ...
    item.statusUpdatedAt = existing?.statusUpdatedAt || now;
    item.statusRevision = existing?.statusRevision || 1;
    ...
    const recurrenceFrequency = type === 'expense' && wantsRecurring ? selectedRecordFrequency() : 'monthly';
    if (type === 'expense' && recurrenceFrequency === 'weekly') {
      item.recurrenceFrequency = 'weekly';
      item.weeklyDay = Number($('#recordWeekday').value || 1);
...
function selectedRecordFrequency() {
  return $('input[name="recordFrequency"]:checked')?.value || 'monthly';
}

function updateRecordRecurrenceUI() {
  const type = $('#recordType').value;
  const config = recordConfig(type);
  const recurring = (type === 'expense' || type === 'income') && $('#recordRecurring').checked;
  const weekly = type === 'expense' && recurring && selectedRecordFrequency() === 'weekly';
  $('#recordFrequencyWrap').hidden = !(type === 'expense' && recurring);
  $('#recordKeepValueWrap').hidden = !recurring;
  $('#recordWeekdayWrap').hidden = !weekly;
  $('#recordDayWrap').hidden = !config.showDay || weekly;
  $('#recordAmountLabel').textContent = weekly ? 'Valor por semana' : 'Valor';
}

function openQuickAmountDialog(type, id) {
  if (!['expense', 'income'].includes(type)) return;
  const item = getCollection(type)?.find(entry => entry.id === id && !entry.deletedAt);
  if (!item) return;
  $('#quickAmountType').value = type;
  $('#quickAmountId').value = id;
  $('#quickAmountTitle').textContent = item.title || 'Registro';
  $('#quickAmountLabel').textContent = type === 'expense' && isWeeklyExpense(item) ? 'Valor por semana' : 'Valor';
  const currentValue = type === 'expense' && isWeeklyExpense(item) ? item.weeklyAmount : item.amount;
  $('#quickAmountInput').value = currentValue == null ? '' : formatNumberInput(currentValue);
  $('#quickAmountDialog').showModal();
  setTimeout(() => {
...
function openFlowDialog(type, id) {
  ...
  $('#flowDecreaseOption').textContent = type === 'receivable' ? 'Recebi' : 'Paguei';
  $('#flowIncreaseOption').textContent = type === 'receivable' ? 'Emprestei mais' : 'Recebi mais';
  $('#flowDecreaseDirection').checked = true;
  $('#flowEntryDate').value = dateStamp();
  $('#flowEntryAmount').value = '';
  renderFlowDialog();
  $('#flowDialog').showModal();
  setTimeout(() => {
    $('#flowEntryAmount').focus();
    $('#flowEntryAmount').select();
  }, 0);
}
...
function renderFlowDialog() {
  ...
  const total = accumulatedFlowAmount(type, item);
  const settled = settledFlowAmount(type, item);
  const remaining = remainingFlowAmount(type, item);
  ...
  $('#flowEntryAmount').disabled = false;
  $('#flowEntryDate').disabled = false;
  $('#flowEntrySubmitBtn').disabled = false;
  updateFlowEntryUI();

  $('#flowHistoryList').innerHTML = entries.length
    ? entries.map(entry => {
      const direction = flowEntryDirection(entry);
      const meta = flowEntryMeta(type, direction);
      return `
      <div class="flow-history-row ${escapeAttr(type)} ${escapeAttr(direction)}">
        <span class="flow-history-icon">${iconSvg(meta.icon)}</span>
        <div class="data-main">
...
function flowEntryMeta(type, direction) {
  if (type === 'receivable') {
    return direction === 'increase'
      ? { historyLabel: 'Emprestou mais', amountLabel: 'Valor emprestado', dateLabel: 'Data do empréstimo', auditAction: 'registrou novo valor a receber', success: 'Novo valor registrado.', sign: '-', icon: 'upload' }
      : { historyLabel: 'Recebeu', amountLabel: 'Valor recebido', dateLabel: 'Data do recebimento', auditAction: 'registrou recebimento', success: 'Recebimento registrado.', sign: '+', icon: 'download' };
  }
  return direction === 'increase'
    ? { historyLabel: 'Recebeu mais', amountLabel: 'Valor recebido', dateLabel: 'Data do recebimento', auditAction: 'registrou novo valor devido', success: 'Novo valor registrado.', sign: '+', icon: 'download' }
    : { historyLabel: 'Pagou', amountLabel: 'Valor pago', dateLabel: 'Data do pagamento', auditAction: 'registrou pagamento', success: 'Pagamento registrado.', sign: '-', icon: 'upload' };
}

function updateFlowEntryUI() {
  const type = $('#flowRecordType').value || 'receivable';
  const direction = $('input[name="flowEntryDirection"]:checked')?.value || 'decrease';
  const meta = flowEntryMeta(type, direction);
  $('#flowEntryAmountLabel').textContent = meta.amountLabel;
  $('#flowEntryDateLabel').textContent = meta.dateLabel;
}
...
function saveFlowEntryFromForm(event) {
  ...
  const direction = $('input[name="flowEntryDirection"]:checked')?.value === 'increase' ? 'increase' : 'decrease';
  const meta = flowEntryMeta(type, direction);
  const remaining = remainingFlowAmount(type, item);
  if (!(amount > 0)) return toast('Informe um valor maior que zero.', 'error');
  if (!date) return toast('Informe a data.', 'error');
  if (direction === 'decrease' && amount > remaining + 0.005) return toast(`O valor máximo é ${formatMoney(remaining)}.`, 'error');
  ...
  addAudit(meta.auditAction, recordEntityLabel(type), `${recordDisplayName(type, item)} - ${formatMoney(amount)} em ${dateOrDash(date)}`);
  saveData();
  $('#flowEntryAmount').value = '';
  $('#flowEntryDate').value = dateStamp();
  renderFlowDialog();
  toast(meta.success, 'good');
}
...
function deleteFlowEntryById(entryId) {
  ...
  if (item && flowEntryDirection(entry) === 'increase') {
    const totalWithoutEntry = roundMoney(accumulatedFlowAmount(entry.recordType, item) - Number(entry.amount || 0));
    if (totalWithoutEntry + 0.005 < settledFlowAmount(entry.recordType, item)) {
      return toast('Este acréscimo não pode ser excluído porque já existem movimentações posteriores sobre esse valor.', 'error');
    }
  }
  if (!confirm(`Excluir este lançamento de ${formatMoney(entry.amount)}?`)) return;
...
function updateFlowRecordStatus(type, item, database = db, touch = false) {
  if (!item) return;
  const complete = remainingFlowAmount(type, item, database) <= 0 && accumulatedFlowAmount(type, item, database) > 0;
...
function cycleRecordStatus(type, id) {
  ...
  const item = getCollection(type).find(entry => entry.id === id);
  if (!item) return;
  if (type === 'expense' && isWeeklyExpense(item)) return changeWeeklyPayment(id, 1);
...
function changeWeeklyPayment(id, direction, kind = 'paid') {
  ...
  if (item.weeklyAmount == null) {
    closeQuickActionsMenu();
    openQuickAmountDialog('expense', id);
    return;
  }
  const now = Date.now();
  const expected = weeklyExpectedCountForItem(item);
...
function upsertRecurrence(id, type, item, keepValue) {
  ...
  Object.assign(recurrence, {
    ...
    frequency: isWeeklyExpense(item) ? 'weekly' : 'monthly',
    weeklyDay: isWeeklyExpense(item) ? item.weeklyDay : null,
    dueDay: item.dueDay,
    day: item.day,
    owner: item.owner,
...
function normalizeFinanceItem(item) {
  ...
  normalized.statusUpdatedAt = Number(item?.statusUpdatedAt || item?.updatedAt || item?.createdAt || 0);
  normalized.statusRevision = Math.max(0, Number(item?.statusRevision || 0));
  return normalized;
}
...
function mergeDB(remote, local) {
  ...
  merged.finances.expenses = mergeFinanceArray(remote.finances.expenses, local.finances.expenses);
...
function mergeFinanceArray(remoteItems = [], localItems = []) {
  ...
  return Array.from(ids).map(id => {
    ...
    const winner = clone(itemStamp(local) >= itemStamp(remote) ? local : remote);
    if (!winner.deletedAt && !remote.deletedAt && !local.deletedAt) {
      const remoteRevision = Number(remote.statusRevision || 0);
      const localRevision = Number(local.statusRevision || 0);
      let statusSource;
      if (localRevision !== remoteRevision) {
        statusSource = localRevision > remoteRevision ? local : remote;
      } else if (local.status !== remote.status && (completedFinanceStatus(local.status) || completedFinanceStatus(remote.status))) {
        statusSource = completedFinanceStatus(local.status) ? local : remote;
      } else {
        statusSource = Number(local.statusUpdatedAt || 0) >= Number(remote.statusUpdatedAt || 0) ? local : remote;
      }
      winner.status = statusSource.status;
      winner.statusUpdatedAt = Number(statusSource.statusUpdatedAt || statusSource.updatedAt || 0);
      winner.statusRevision = Number(statusSource.statusRevision || 0);
      if (isWeeklyExpense(remote) || isWeeklyExpense(local)) {
        winner.weeklyPayments = mergeArray(remote.weeklyPayments || [], local.weeklyPayments || []);
        reconcileWeeklyExpenseStatus(winner);
      }
    }
    return winner;
  });
...
function completedFinanceStatus(status) {
...
function formatMoneyInputAsCents(input) {
